...
class TreeContructor {
    constructor(element, rightPane) {
        ...
        //}).value();
        self.tableDictionary = new Array();
    ...
    generateNodes(el, rightPane, nodes) {
        var self = this;
        ...
        if (container != null && container.Items != null) {
            container.Items.forEach(function (item) {
                if (item instanceof Gemini.Param) {
                    ...
                    parameters = parameters.concat(self.getAllParams(item));
                }
            });
        }
        return parameters;
    ...
    //    return params;
    //};
    generateObjects(sectionDiv, objectsArray, params) {
        var self = this;
        objectsArray.forEach(function (value) {
            var div = document.createElement('div');
            div.classList.add('row');
            var controlUID = value.Attached;
            var param = (_.first(_.where(params, { UID: controlUID })));
    ...
    generateTable(sectionDiv, table, tableParams) {
        var self = this;
        ...
        table.GuixTableColumn.forEach(function (value) {
            var param = (_.first(_.where(tableParams, { UID: value.Attached })));
            var column = {
                title: param.Name,
                field: param.MibName
            };
    ...
    generateSection(divSection, guixSection) {
        ...
        headerLabel.innerText = (guixSection.Name);
                ...
                if (guixSection.Items != null) {
                    for (var i = 0; i < guixSection.Items.length; i++) {
                        if (guixSection.Items[i] instanceof Gemini.GuixTable) {
                            this.generateTable(sectionDiv, (guixSection.Items[i]), params);
                        }
                    }
                    var parameters;
                    parameters = new Array();
                    ...
                    for (var i = 0; i < guixSection.Items.length; i++) {
                        if (guixSection.Items[i] instanceof Gemini.GuixObject) {
                            parameters[j++] = (guixSection.Items[i]);
                        }
                    }
